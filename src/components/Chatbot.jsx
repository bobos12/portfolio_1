...
// The system prompt and the Groq key now live server-side in api/chat.js.
  ...
  const [messages, setMessages] = useState([
    {
      id: 0,
      role: "assistant",
      content: "Hey, welcome 👋 I'm Ahmed — full-stack developer, 4+ years building production apps. Ask me about my work, my stack, or how we can work together.",
    },
  ]);
  ...
  const send = useCallback(async (text) => {
    ...
    try {
      ...
      generation = traceRef.current?.generation({
        ...
        input: history,
        modelParameters: { max_tokens: 350, temperature: 0.72, reasoning_effort: "low" },
      });
      ...
      if (!res.ok) throw new Error(`Chat ${res.status}: ${data?.error ?? "request failed"}`);
      ...
      console.error("%c❌ Chat request failed", "color:#f87171;font-weight:bold", err);
